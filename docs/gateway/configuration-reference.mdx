...
See [Customize the bind address and port](/deployment/tensorzero-gateway#customize-the-bind-address-and-port) for more details.
...
The time-to-live (TTL) in seconds for inference cache entries stored in Valkey.
By default, cache entries expire after 24 hours (86400 seconds).
This setting only applies when Valkey is available (i.e. `TENSORZERO_VALKEY_URL` is set); otherwise, the inference cache uses ClickHouse.
...
If you use a single Valkey instance for both rate limiting and caching, see [Inference Caching](/gateway/guides/inference-caching#cache-backend) for recommended TTL settings.
...
### `export.otlp.traces.include_content`

- **Type:** boolean
- **Required:** no (default: `false`)

When `true` and `format = "opentelemetry"`, emit the content-carrying GenAI semantic-convention attributes (`gen_ai.input.messages`, `gen_ai.output.messages`, `gen_ai.system_instructions`, `gen_ai.tool.definitions`) on the `model_provider_inference` span. These attributes contain the full prompt and response content, and are opt-in even when trace export is enabled.

When `format = "openinference"`, setting this field has no effect.
...
By default, the gateway offloads the responsibility of writing inferences, feedback, and other data to your database to a background task, instead of waiting for the database to complete the writes.
Each database insert is handled immediately in separate background tasks.

To disable async writes and use synchronous writes instead, set `gateway.observability.async_writes = false`.

See the ["Optimize latency and throughput" guide](/deployment/optimize-latency-and-throughput) for best practices.

You can't enable `async_writes` and `batch_writes` at the same time. Since `async_writes` is enabled by default, you must explicitly disable it to use `batch_writes`:

```toml title="tensorzero.toml"
[gateway]
# ...
observability.async_writes = false
observability.batch_writes = { enabled = true }
# ...
```

<Warning>

Since async writes are enabled by default, make sure that the gateway lives long enough to complete the writes.
...
Enabling this setting will improve the latency and throughput of the gateway by offloading the responsibility of writing inferences, feedback, and other data to your database to a background task, instead of waiting for the database to complete the writes.
...
- `max_rows_postgres` (integer, optional): Maximum number of rows to collect before flushing a Postgres batch (default: `max_rows`)
- `write_queue_capacity` (integer, optional): Maximum number of inferences each gateway instance can hold in memory per table type before dropping new rows to protect against out-of-memory crashes. When set, the write queue is bounded: if full, new rows are dropped and an error is logged. When unset (default), the queue is unbounded — all rows are buffered in memory. We recommend setting this for high-throughput production deployments, and monitoring your gateway instances' memory usage to determine an appropriate value
...
If `true`, the gateway will throw an error on startup if it fails to validate the database connection.
If `null`, the gateway will log a warning but continue if a database is not available, and it will use the database if available (if both Postgres and ClickHouse are available, the gateway will use ClickHouse as the observability backend).
If `false`, the gateway will not enable observability.
...
```toml title="tensorzero.toml" mark="openai" mark="azure"
[models.gpt-4o]
# ...
...
- `timeouts.non_streaming.total_ms` — the total time allowed for a non-streaming request.
- `timeouts.streaming.ttft_ms` — the time allowed to receive the first token (TTFT) in a streaming request.
- `timeouts.streaming.total_ms` — the total time allowed for the entire streaming request (measured from request start).

For example, the following configuration sets a 15-second timeout for non-streaming requests, a 3-second TTFT timeout, and a 60-second total timeout for streaming requests:
...
Scopes this model to a specific namespace.
When set, the model can only be used by variants that are reachable from a matching namespace experimentation config.
This is validated at both config time and inference time.

This is useful for per-customer fine-tuned models that should only be served when the correct namespace is provided at inference time.

```toml title="tensorzero.toml"
[models.customer-a-ft]
routing = ["fireworks"]
namespace = "customer_a"

[models.customer-a-ft.providers.fireworks]
type = "fireworks"
...
```toml title="tensorzero.toml" mark="gpt-4o" mark="openai" mark="azure"
[models.gpt-4o]
# ...
routing = ["openai", "azure"]
# ...
...
Configures how to extract cost information from batch inference provider responses.
This is used for cost tracking of [batch inferences](/gateway/guides/batch-inference).

Many providers offer discounted pricing for batch inferences compared to real-time inferences.
Use `batch_cost` to configure the batch-specific rates.
...
- `pointer` (string) — a [JSON Pointer](https://datatracker.ietf.org/doc/html/rfc6901) into the provider's batch response.
- Rate (exactly one of the following):
  - `cost_per_million` (number) — cost in dollars per million units of the extracted value.
  - `cost_per_unit` (number) — cost in dollars per unit of the extracted value.
- `required` (boolean, default: `false`) — if `true` and the field is missing from the response, the cost for the entire request is reported as `null`. If `false`, missing fields contribute $0 to the total.

The total cost is computed as the sum of `extracted_value * rate` across all entries.
...
cost = [
  { pointer = "/usage/prompt_tokens", cost_per_million = 2.50, required = true },
  { pointer = "/usage/completion_tokens", cost_per_million = 10.00, required = true },
  { pointer = "/usage/prompt_tokens_details/cached_tokens", cost_per_million = -1.25 },
]
# Batch inference pricing (50% discount)
batch_cost = [
...
Configures how to extract cost information from raw provider responses.
This is used for cost tracking and [cost-based rate limiting](/operations/enforce-custom-rate-limits).

Each entry in the array specifies a [JSON Pointer](https://datatracker.ietf.org/doc/html/rfc6901) to a numeric field in the provider's response, a rate to convert that value to dollars, and whether the field is required.

Each entry has the following fields:

- Pointer (exactly one of the following):
  - `pointer` — a single JSON Pointer used for both streaming and non-streaming responses.
  - `pointer_nonstreaming` and `pointer_streaming` — separate JSON Pointers for non-streaming and streaming responses respectively. Both must be provided together.
- Rate (exactly one of the following):
  - `cost_per_million` (number) — cost in dollars per million units of the extracted value. This is the most common option for token-based pricing.
  - `cost_per_unit` (number) — cost in dollars per unit of the extracted value. This is useful for features billed per unit (e.g. web search calls), or when the provider response contains a cost value directly (set to `1.0`).
- `required` (boolean, default: `false`) — if `true` and the field is missing from the response, the cost for the entire request is reported as `null`. If `false`, missing fields contribute $0 to the total.
...
<Accordion title="Example: Token-based pricing (e.g. OpenAI)">

For providers that report token counts, configure cost entries with `cost_per_million`.
Include optional fields for usage that may vary (e.g. cached input tokens).
OpenAI includes cached tokens in `prompt_tokens`, so use a negative rate to subtract the discount.
...
cost = [
  { pointer = "/usage/prompt_tokens", cost_per_million = 2.50, required = true },
  { pointer = "/usage/completion_tokens", cost_per_million = 10.00, required = true },
  # OpenAI includes cached tokens in prompt_tokens but bills them at half price.
  # Use a negative rate to subtract the difference.
  { pointer = "/usage/prompt_tokens_details/cached_tokens", cost_per_million = -1.25 },
]
```
...
<Accordion title="Example: Split pointers for streaming vs. non-streaming">

Some providers report usage differently in streaming and non-streaming responses.
Use `pointer_nonstreaming` and `pointer_streaming` to handle this:
...
model_name = "my-model"
cost = [
  { pointer_nonstreaming = "/usage/prompt_tokens", pointer_streaming = "/usage/prompt_tokens", cost_per_million = 3.00, required = true },
  { pointer_nonstreaming = "/usage/completion_tokens", pointer_streaming = "/usage/completion_tokens", cost_per_million = 15.00, required = true },
]
```

</Accordion>

<Accordion title="Example: Per-unit pricing (e.g. web search)">

For features billed per invocation (e.g. web search calls), use `cost_per_unit` to set the price per occurrence.
This example configures Anthropic's web search tool at $10 per 1,000 searches ($0.01 per search):
...
cost = [
  ...
  # Anthropic reports cached tokens separately (not included in input_tokens),
  # so add them at their discounted rates.
  ...
  { pointer = "/usage/server_tool_use/web_search_requests", cost_per_unit = 0.01 },
]
```
...
You can also set timeouts at the model level and variant level.
Multiple timeouts can be active simultaneously.
...
When specified, `secret_access_key` must also be provided.
Both fields must use the same source type (both `env::`, both `dynamic::`, or both `sdk`).

You can't combine this field with `api_key`.
...
API key for bearer token authentication. When set, TensorZero uses `Authorization: Bearer <token>` instead of AWS SigV4 signing.
...
If neither `api_key` nor IAM credentials are configured, TensorZero checks for `AWS_BEARER_TOKEN_BEDROCK` environment variable first.
If found, it uses bearer token authentication.
Otherwise, it falls back to the AWS SDK credential chain.

You can't combine this field with IAM credentials like `access_key_id`.
...
When specified, `access_key_id` must also be provided.
Both fields must use the same source type (both `env::`, both `dynamic::`, or both `sdk`).
...
When specified, `access_key_id` and `secret_access_key` must also be provided.
All three fields must use the same source type (all `env::`, all `dynamic::`, or all `sdk`).
...
```toml title="tensorzero.toml"
[models."gpt-5-mini".providers.azure]
# ...
type = "azure"
...
When set to `"parsed"`, Groq will return structured reasoning content (thinking) separately from the main response.
This should only be set for reasoning models that support the `reasoning_format` parameter (e.g. `qwen/qwen3-32b`).
Non-reasoning models will reject requests with this parameter.

```toml title="tensorzero.toml"
[models.qwen3_32b.providers.groq]
# ...
type = "groq"
...
Controls reasoning behavior for Mistral's Magistral reasoning models.
Set to `"reasoning"` to enable reasoning output with models like `magistral-small-latest` and `magistral-medium-latest`.

```toml title="tensorzero.toml"
[models.magistral_small.providers.mistral]
# ...
type = "mistral"
...
Configures how to extract cost information from raw embedding provider responses.
This is used for cost tracking and [cost-based rate limiting](/operations/enforce-custom-rate-limits).
...
- `pointer` (string) — a [JSON Pointer](https://datatracker.ietf.org/doc/html/rfc6901) into the provider's response.
- Rate (exactly one of the following):
  - `cost_per_million` (number) — cost in dollars per million units of the extracted value. This is the most common option for token-based pricing.
  - `cost_per_unit` (number) — cost in dollars per unit of the extracted value. This is useful for features billed per unit, or when the provider response contains a cost value directly (set to `1.0`).
- `required` (boolean, default: `false`) — if `true` and the field is missing from the response, the cost for the entire request is reported as `null`. If `false`, missing fields contribute $0 to the total.
...
It helps TensorZero Autopilot understand the intent behind the function.
...
```toml title="tensorzero.toml" mark="run-python"
...
```toml title="tensorzero.toml" mark="query-database"
...
```toml title="tensorzero.toml" mark="draft-email"
[functions.draft-email]
# ...
...
For Anthropic, this enables adaptive thinking (`thinking.type: "adaptive"`) with the specified effort level via `output_config.effort`.
...
For Anthropic, `reasoning_effort` and `thinking_budget_tokens` are mutually exclusive. Use `reasoning_effort` for adaptive thinking or `thinking_budget_tokens` for manual thinking.
...
For Anthropic, this value corresponds to `thinking.budget_tokens` (manual thinking mode with `thinking.type: "enabled"`).
...
For Anthropic, `thinking_budget_tokens` and `reasoning_effort` are mutually exclusive. Use `thinking_budget_tokens` for manual thinking or `reasoning_effort` for adaptive thinking.

</Warning>

<Tip>

Some providers (e.g. OpenAI) support `reasoning_effort` instead. For Anthropic, both `reasoning_effort` and `thinking_budget_tokens` are supported but cannot be used together.
...
Evaluators are defined as part of a function's configuration and specify how to evaluate the function's outputs.
You can define multiple evaluators by including multiple `[functions.function_name.evaluators.evaluator_name]` sections.

Evaluators are used by [TensorZero Evaluations](/evaluations/inference-evaluations/tutorial) to score and compare variants.
...
See the [Evaluations Configuration Reference](/evaluations/inference-evaluations/configuration-reference) for the full list of evaluator types and their configuration options.
...
<Accordion title='type: "static"'>

The `static` type samples variants according to fixed probabilities.
...
You can specify `candidate_variants` as either a list of variant names (uniform sampling) or a map of variant names to weights (weighted sampling), and `fallback_variants` for sequential fallback behavior.

### `candidate_variants`

- **Type:** array of strings _or_ map of strings to floats
- **Required:** yes

The set of variants to sample from.
Each variant must be defined via `[functions.function_name.variants.variant_name]` in the `variants` sub-section.

When specified as a **list** of variant names, each variant is sampled with equal probability (uniform sampling).
...
When specified as a **map** of variant names to weights, variants are sampled according to their relative weights.
...
**Explicit candidate variants (uniform):**
...
**Weighted candidate variants:**

```toml title="tensorzero.toml"
[functions.draft-email.experimentation]
type = "static"
candidate_variants = {"variant-a" = 0.9, "variant-b" = 0.1}
```
...
<Accordion title='type: "adaptive"'>

The `adaptive` type samples variants according to probabilities that dynamically update based on accumulating feedback data.
It is designed to efficiently identify the best variant with a specified level of confidence.
...
Each variant must be defined via `[functions.function_name.variants.variant_name]` in the `variants` sub-section (see above).
...
The algorithm aims to identify a "winner" variant that has the best average value for the chosen metric, or nearly the best (where "best" means highest if `optimize = "max"` or lowest if `optimize = "min"` for the chosen metric, and "nearly" is determined by a tolerance `epsilon`, defined below).
...
The algorithm aims to identify a "winner" variant whose average metric value is either the highest, or within epsilon of the highest.
...
Feedback for fallback variants will not be used in the experiment itself; the sampling probabilities will be dynamically updated based only on feedback for the `candidate_variants`.
...
The aim of the adaptive algorithm is to identify an epsilon-best variant, without necessarily differentiating sub-optimal variants, so the primary use for this field is to enable the user to ensure that sufficient data is gathered to learn about the performance of sub-optimal variants.
Note that this field has no effect once the algorithm picks a winner variant, since at that point random sampling ceases and the winner variant is used exclusively.
...
Updating the sampling probabilities requires reading the latest feedback data from your database.
...
You can override the base experimentation config for specific namespaces using the `namespaces` sub-section.
...
```toml title="tensorzero.toml"
# Base config: used when no namespace is provided or namespace has no specific config
[functions.draft-email.experimentation]
type = "static"
candidate_variants = ["variant-a", "variant-b"]

# Mobile namespace: always serve variant-a
[functions.draft-email.experimentation.namespaces.mobile]
type = "static"
candidate_variants = {"variant-a" = 1.0}

# Web namespace: run an adaptive experiment
[functions.draft-email.experimentation.namespaces.web]
type = "adaptive"
candidate_variants = ["variant-a", "variant-b"]
metric = "user-rating"
```
...
It helps TensorZero Autopilot understand the intent behind the metric.
...
```toml title="tensorzero.toml" mark="get-temperature"
...
Postgres is required for certain TensorZero features including [adaptive experimentation](/experimentation/run-adaptive-ab-tests/) and [authentication](/operations/set-up-auth-for-tensorzero).
Postgres can also be used for [rate limiting](/operations/enforce-custom-rate-limits/), though [Valkey](/deployment/valkey-redis) is recommended for high-throughput deployments.
...
- **Type:** number (dollars)
- **Required:** no (default: `1.00`)

The default cost in dollars used to estimate each request's cost upfront when cost rate limiting is active.
After a request completes, the gateway adjusts the consumed budget based on the actual cost reported by the provider.
...
**Simple Format:** A single value that sets both the capacity and refill rate to the same value.
For `model_inferences` and `tokens`, this is an integer. For `cost`, this is a number in dollars.
