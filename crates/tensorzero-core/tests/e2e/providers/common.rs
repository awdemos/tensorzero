...
pub async fn check_tool_use_tool_choice_none_inference_response(
    ...
    if content.is_empty() {
        assert!(
            allows_empty_batch_content && output_tokens.is_null(),
            "Expected null output tokens only for permitted empty batch content, got {output_tokens:?}"
        );
    } else {
        assert!(output_tokens.as_u64().unwrap() > 0);
    }
    ...
    if content.is_empty() {
        assert!(
            allows_empty_batch_content && output_tokens.is_null(),
            "Expected null stored output tokens only for permitted empty batch content, got {output_tokens:?}"
        );
    } else {
        assert!(output_tokens.as_u64().unwrap() > 0);
    }
